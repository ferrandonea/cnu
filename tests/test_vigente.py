...
def test_vigente_2026_selecciona_tm2020():
    ...
    assert cnu.cnu_afiliado(65, agno_actual=2026) == cnu.cnu_afiliado(65, tabla="cb2020", agno_actual=2026)
    assert cnu.cnu_conyuge(65, 63, cony_mujer=True, agno_actual=2026) == cnu.cnu_conyuge(
        65, 63, cony_mujer=True, tabla="cb2020", tabla_benef="b2020", agno_actual=2026
    )
    assert cnu.cnu_sobrevivencia_conyuge(63, mujer=True, agno_actual=2026) == cnu.cnu_sobrevivencia_conyuge(
        63, mujer=True, tabla_benef="b2020", agno_actual=2026
    )


def test_vigente_sin_agno_actual_usa_el_agno_en_curso():
    esperado = cnu.cnu_afiliado(65, agno_actual=cnu.core.agno_actual_por_defecto())
    assert cnu.cnu_afiliado(65) == esperado
...
def test_convencion_fin_de_agno():
    ...
    assert cnu.cnu_afiliado(65, agno_actual=2023) == cnu.cnu_afiliado(65, tabla="cb2020", agno_actual=2023)
    assert cnu.cnu_afiliado(65, fsiniestro=20230630, agno_actual=2023) == cnu.cnu_afiliado(
        65, tabla="cb2014", agno_actual=2023
    )
...
def test_tabla_explicita_se_respeta():
    ...
    assert cnu.cnu_afiliado(65, tabla="rv2009", agno_actual=2026) == cnu.cnu_afiliado(
        65, tabla="rv2009", agno_actual=2026, fsiniestro=0
    )
    assert cnu.cnu_afiliado(65, tabla="rv2009", agno_actual=2026) != cnu.cnu_afiliado(65, agno_actual=2026)


def test_faj_proyeccion_y_vectoriales_usan_la_misma_resolucion():
    kw = dict(agno_actual=2026)
    exp = dict(tabla="cb2020", tabla_benef="b2020", agno_actual=2026)
    assert cnu.faj_afiliado(65, 62, **kw) == cnu.faj_afiliado(65, 62, **exp)
    ...
    v = cnu.cnu_afiliado_vec([65, 65, 65], mujer=[False, True, False], agno_actual=[2026, 2026, 2013])
    esperado = [
        cnu.cnu_afiliado(65, tabla="cb2020", agno_actual=2026),
        cnu.cnu_afiliado(65, mujer=True, tabla="rv2020", agno_actual=2026),
        cnu.cnu_afiliado(65, tabla="rv2009", agno_actual=2013),
    ]
    ...
    assert v[0] == pytest.approx(cnu.cnu_sobrevivencia_conyuge(62, mujer=True, tabla_benef="b2020", agno_actual=2026))
    v = cnu.faj_afiliado_vec([65, 65], [62, 62], cony_mujer=True, rp=0.03, agno_actual=[2026, 2014])
    assert v[0] == pytest.approx(cnu.faj_afiliado_vec([65], [62], cony_mujer=True, rp=0.03, **exp)[0])
...
def test_cli_default_vigente(capsys):
    ...
    assert main(["afil", "65", "--agno-actual", "2026"]) == 0
    out = capsys.readouterr().out
    assert "tabla cb2020h" in out
    assert float(out.strip().splitlines()[-1]) == cnu.cnu_afiliado(65, tabla="cb2020", agno_actual=2026)
    main(["conyuge", "65", "63", "--agno-actual", "2026"])
    assert "tablas cb2020h b2020m" in capsys.readouterr().out
    main(["sobrev", "63", "--mujer", "--agno-actual", "2023"])
    assert "tabla b2020m" in capsys.readouterr().out
    main(["proy", "65", "62", "--agno-actual", "2026"])
    assert "cb2020 b2020" in capsys.readouterr().out
    main(["faj", "65", "--agno-actual", "2026"])
    assert float(capsys.readouterr().out.strip().splitlines()[-1]) == pytest.approx(
        cnu.faj_afiliado(65, tabla="cb2020", agno_actual=2026), abs=1e-6
    )
